...
dados <-read.csv(file="amostra_enem2012_50k.csv", sep =",")
dados$X <- NULL
ingles <- c("X","X","X","X","X")
...
for(i in 1:nrow(dados)){
  ...
  if(dados[i,13] == 0){
    ...
    codigo_prova_ingles <- rbind(codigo_prova_ingles,data.frame(dados[i,11]))
  }else{
    for(k in 1:5){
      vet_respostas[k]<-str_sub(resposta,k,k)
    }
    espanhol <- rbind(espanhol,vet_respostas[1:5])
    codigo_prova_espanhol <- rbind(codigo_prova_espanhol,data.frame(dados[i,11]))
  }
  print(i)
