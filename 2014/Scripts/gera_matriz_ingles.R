...
gabarito_ingles <- c("A","C","A","C","D")

for(i in 1:nrow(dados)){
  contadorAux <- contadorAux + 1
  vet_respostas <- dados[i,1:5]

  if(dados[i,6]==205){ #prova azul
    ...
    if(dados[i,6]==203){ #prova amarela
      ...
      if(gabarito_ingles[5]==vet_respostas[2]){
        respostas_LC[contadorAux,5]<-1
      }
    }
    
    if(dados[i,6]==206){ #prova rosa
      if(gabarito_ingles[1]==vet_respostas[3]){
        respostas_LC[contadorAux,1]<-1
      }
