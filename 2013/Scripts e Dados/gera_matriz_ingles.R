...
gabarito_ingles <- c("C","C","A","B","C")

for(i in 1:nrow(dados)){
  contadorAux <- contadorAux + 1
  vet_respostas <- dados[i,1:5]

  if(dados[i,6]==175){ #prova amarela
    ...
    if(dados[i,6]==178){ #prova rosa
      ...
      if(gabarito_ingles[3]==vet_respostas[5]){
        respostas_LC[contadorAux,3]<-1
      } 
      
      if(gabarito_ingles[4]==vet_respostas[3]){
        respostas_LC[contadorAux,4]<-1
      } 
    ...
    if(dados[i,6]==177){ #prova azul
      if(gabarito_ingles[1]==vet_respostas[5]){
        respostas_LC[contadorAux,1]<-1
      }
      
      if(gabarito_ingles[2]==vet_respostas[4]){
        respostas_LC[contadorAux,2]<-1
      } 
      ...
      if(gabarito_ingles[4]==vet_respostas[2]){
        respostas_LC[contadorAux,4]<-1
      } 
      
      if(gabarito_ingles[5]==vet_respostas[3]){
        respostas_LC[contadorAux,5]<-1
      }
    }
    
    if(dados[i,6]==176){ #prova cinza
      if(gabarito_ingles[1]==vet_respostas[4]){
        respostas_LC[contadorAux,1]<-1
      }
      
      if(gabarito_ingles[2]==vet_respostas[5]){
        respostas_LC[contadorAux,2]<-1
      } 
