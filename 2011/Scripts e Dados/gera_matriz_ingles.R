...
gabarito_ingles <- c("A","D","D","B","B")

for(i in 1:nrow(dados)){
  contadorAux <- contadorAux + 1
  vet_respostas <- dados[i,1:5]

  if(dados[i,6]==146){ #prova cinza
    ...
    if(dados[i,6]==145){ #prova amarela
    ...
    if(dados[i,6]==148){ #prova rosa
      ...
      if(gabarito_ingles[4]==vet_respostas[1]){
        respostas_LC[contadorAux,4]<-1
      } 
